...
def run(
    ...
    logger.info(
        f"Iniciando Experimento VRP (N={n_cities}, Vehicles={num_vehicles}, Layers={layers}, "
        f"Reps={reps}, Opt={optimizer_method}, LR={lr}, Device={device.upper()}, MaxIter={maxiter})"
    )

    constructor_info_name = (
        f"N{n_cities}_Vehicles{num_vehicles}_Layers={layers}_Reps={reps}_"
        f"Opt={optimizer_method}_LR={lr}_Dev={device.upper()}_MaxIter={maxiter}"
    )
    ...
    # 4. SOLVER VQE
    logger.info(f"4. Otimizando circuito CV-VQE ({optimizer_method} | lr={lr} | device={device.upper()})...")
    t0 = time.time()
    ...
    print("\n" + "="*70)
    print("                     RESULTADOS FINAIS DO VRP                     ")
    print("="*70)
