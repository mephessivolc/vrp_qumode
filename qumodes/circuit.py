...
class Circuit:
    ...
    def initialize_warm_start_params(
        ...
        # 1. Normalização de target_routes para a estrutura Dict[int, List[int]]
        routes_dict: Dict[int, List[int]] = {}
        if isinstance(target_routes, dict):
            routes_dict = target_routes
        elif isinstance(target_routes, list):
            if len(target_routes) > 0 and isinstance(target_routes[0], list):
                # Lista de rotas por veículo: [[0, 1, 0], [0, 2, 3, 0]]
                routes_dict = {v_idx + 1: route for v_idx, route in enumerate(target_routes)}
            else:
                # Rota única (TSP/Single-Vehicle): [0, 1, 2, 0]
                routes_dict = {1: target_routes}
                
        # Garantir presença de todas as chaves de veículos
        for v_idx in range(1, num_vehicles + 1):
            if v_idx not in routes_dict:
                routes_dict[v_idx] = [0, 0]

        # 2. Mapeamento de cada cidade livre (1 até N-1) para seu (x_target, p_target)
        ...
        for step in range(total_steps):
            ...
            # c) Displacement Gate (Injeção de Warm-Start na camada 0)
