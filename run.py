...
from utils import format_timespan, plot_convergence
...
def run_experiment(
    ...
    fig_name: Optional[str] = "test_convergence",
):
    ...
    tenho que inserir o Bruteforce para comparar resultados, salvar em json unico todos os resultados de forma que um unico aruivo tenha todas as simulações
    ...
    print(
        f"Tempo de Execução: {format_timespan(metrics.execution_time_seconds)}"
    )
    ...
    if fig_name is not None:
        fig_path_name = path.get_file_path(fig_name, is_result=False)
        plot_convergence(
            metrics=metrics, method=method, fig_path_name=fig_path_name
        )
